...
class Cell:
    def __init__(self, quantity):
        try:
            if quantity < 0 and not quantity.isdigit():
                raise AttributeError
            self.quantity = quantity
        except AttributeError:
            print('Это должно быть целое положительное число!')
            print('Создается клетка с минимальным значением "1"')
            self.quantity = 1
    ...
    def __sub__(self, other):
        return f'Результат операции {(self.quantity - other.quantity) * "*"}' if (
                                                                                             self.quantity - other.quantity) > 0 else 'Ушли в минус :(  '
