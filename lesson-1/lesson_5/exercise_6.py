...
my_dict = {}
with open('for_sixth_exercise.txt', encoding='utf-8') as f:
    for ls in f.readlines():
        info_subject = ls.split()
        sum_list = [int(i[:i.find('(')]) for i in info_subject[1:] if i[:i.find('(')].isdigit()]
        my_dict.update({info_subject[0][0:-1]: sum(sum_list)})
    print(my_dict)
